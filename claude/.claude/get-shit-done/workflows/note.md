...
- **Global scope**: `/Users/elian.ortega.somnio/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/elian.ortega.somnio/.claude/notes/`)
...
2. Glob `/Users/elian.ortega.somnio/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/elian.ortega.somnio/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/elian.ortega.somnio/.claude/notes/` — works in any directory
